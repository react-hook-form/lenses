...
test('map callback accepts a keyName and index', () => {
  ...
  const itemLenses = result.current.lens.focus('items').map(
    [
      { a: '1', myId: 'one' },
      { a: '2', myId: 'two' },
    ],
    (l, keyName, index) => ({ interop: l.interop(), keyName, index }),
    'myId',
  );

  expect(itemLenses[0]).toEqual({ interop: { name: 'items.0', control: result.current.form.control }, keyName: 'one', index: 0 });
  expect(itemLenses[1]).toEqual({ interop: { name: 'items.1', control: result.current.form.control }, keyName: 'two', index: 1 });
});
